...
class SortingMethods {
    ...
    static int[] insertionSort(int arr[])  //inserting elements in sorted way prefrable if array is almost sorted ie.for best case
  {
      for(int i=1;i<arr.length;i++){ //shifting larger elements to next position and inserting key element to pcorrect location 
          int key=arr[i];
          int j=i-1;
          while(j>=0 && arr[j]>key){
              arr[j+1]=arr[j];
  ...
  static void mergeSort(int arr[],int low,int high){ //prefered worst case=nlogn
      if(low<high){
          int mid=low+(high-low)/2;      //this is prefered over (low+high)/2 because low+high may cause overflow ie.if it is greater than maxinteger
